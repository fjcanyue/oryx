...
use crate::doc::mermaid::{self, MermaidPresentation};
...
pub trait Pictures {
    ...
    fn mermaid(&mut self, _source: &str) -> Option<Picture> {
        None
    }
}
    ...
    fn block(&mut self, index: usize, cut: &Cut) {
        ...
        match &block.kind {
            ...
            BlockKind::Mermaid { body } => {
                let lines: Vec<_> = body.iter(source).collect();
                let selected: Vec<_> = lines
                    .iter()
                    .enumerate()
                    .filter_map(|(index, line)| cut.slice(index, line))
                    .collect();
                ...
                self.enter(block);
                if selected == lines {
                    if let Some(picture) = self.pictures.mermaid(&text) {
                        let size = (Some(picture.width), Some(picture.height));
                        let _ = write!(self.out, "<p>{}</p>", img(&picture, &text, "", size));
                        return;
                    }
                }
                let _ = write!(self.out, "<pre>{}</pre>", escape(&text));
            }
...
impl Pictures for PagePictures<'_> {
    fn mermaid(&mut self, source: &str) -> Option<Picture> {
        let presentation = MermaidPresentation::from_oryx(self.theme);
        let uri = mermaid::cache_key(source, &presentation).uri();
        if self.media.generated(&uri).is_none() {
            let rendered = mermaid::render(source, &presentation).ok()?;
            self.media.register_generated_svg(
                uri.clone(),
                rendered.svg,
