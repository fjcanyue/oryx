...
pub const TIPS: &[Tip] = &[
    ...
    Tip {
        kind: TipKind::Tip,
        area: Area::Finding,
        text: r#"`Ctrl+Shift+F` searches the files in the sidebar's folder. Results open at the matching line. To find a file by its name or path, press `/` in the Files tab instead."#,
    },
    Tip {
        kind: TipKind::Tip,
        area: Area::Finding,
...
mod tests {
    ...
    #[test]
    fn reference_frontmatter_handles_lf_and_crlf_without_changing_the_body() {
        for newline in ["\n", "\r\n"] {
            let body = format!("# Syntax{newline}{newline}Text{newline}");
            let source = format!("---{newline}title: Syntax{newline}---{newline}{body}");
            assert_eq!(strip_frontmatter(&source), body);
            assert_eq!(
                strip_frontmatter(&format!("---{newline}---{newline}{body}")),
                body
            );
            assert_eq!(strip_frontmatter(&body), body);
            let incomplete = format!("---{newline}title: Syntax{newline}");
    ...
    #[test]
    fn the_mermaid_reference_keeps_both_the_diagram_and_its_literal_example() {
        for source in [SYNTAX.to_string(), page()] {
            let doc = crate::doc::markdown::parse(source);
            assert!(doc
                .blocks
                .iter()
                .any(|block| matches!(block.kind, BlockKind::Mermaid { .. })));
            assert!(doc.blocks.iter().any(|block| match &block.kind {
                BlockKind::CodeBlock { lines, .. } => {
                    lines.iter(&doc.source).any(|line| line == "```mermaid")
                }
                _ => false,
            }));
        }
